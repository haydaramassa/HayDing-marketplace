...
function Products() {
      ...
      try {
        ...
        if (isLoggedIn()) {
          ...
          const ids = Array.isArray(favorites)
            ? favorites
                .map((item) =>
                  normalizeFavoriteId(item.productId || item.product?.id || item.id)
                )
                .filter((id) => id !== null)
            : [];
  ...
  const displayedProducts = isMobileCatalog
    ? visibleProducts.slice(0, visibleProductCount)
    : visibleProducts;

  const canShowMoreProducts =
    isMobileCatalog && visibleProductCount < visibleProducts.length;
  ...
  return (
    ...
    >
      ...
      <main className="products-page-main">
          ...
          <div className="products-mobile-filter-bar">
            ...
            >
              ...
              <span className="products-mobile-filter-count">
                ...
                {text("Anzeigen", "إعلانات", "listings")}
              </span>
        ...
        {!isLoading && !error && canShowMoreProducts && (
          <div className="products-see-more-wrap">
            <button
              className="products-see-more-btn"
              type="button"
              onClick={() =>
                setVisibleProductCount((currentCount) => currentCount + 10)
              }
            >
              {text("Mehr anzeigen", "عرض المزيد", "See more")}
            </button>
          </div>
        )}
      </main>
